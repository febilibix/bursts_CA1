...
def run_simulation(alpha, a, lr, ma_pc, mb_pc, W_ip_a, W_ip_b, tau_a, lr_inh):
    # lr = 20 # 10
    t_epoch = 1
    speed = 20
    len_edge = 50 # 50
    ...
    for condition in ['exp', 
        ...
        pyramidal = PyramidalCells(n_cells, len_edge = len_edge, learning_rate = lr, dt=dt, seed = seed, n_dim=2, inh_plasticity=True)
        ...
        pyramidal.W_ip_b = 2*W_ip_b*np.random.rand(n_cells['inter_b'], n_cells['pyramidal'])/n_cells['pyramidal']
        ...
        for out in ENVIRONMENTS_RUNS.keys():
            ...
            if os.path.exists(f'data/act_maps_{condition}_{out}_{W_ip_a}_{alpha}_{lr}.pkl'):
                print(f"Data for {condition} with W_ip_a={W_ip_a}, alpha={alpha}, lr={lr} already exists. Skipping...")
                continue
            ...
            with open(f'data/act_maps_{condition}_{out}_{W_ip_a}_{alpha}_{lr}.pkl', 'wb') as f:
