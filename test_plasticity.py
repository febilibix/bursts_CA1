...
def run_simulation(n_pyr, n_int_a, n_int_b, n_CA3, tn, lr, n_patterns):
    ...
    pyramidal.learn_patterns(patterns, top_down, n_presentations=n_presentations, t_per_pattern=t_per_pattern, dt=dt)
    pyramidal.pattern_retrieval(patterns, top_down, t_per_pattern=t_per_pattern, dt=dt)

    tn = n_patterns * t_per_pattern * (n_presentations + 1)

    event_count, burst_count = pyramidal.spike_count, pyramidal.burst_count

    delta_t = 10
    neuron_type = np.repeat(top_down, (n_presentations+1)*t_per_pattern/delta_t, axis=1).T
